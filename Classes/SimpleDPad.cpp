...
SimpleDPad* SimpleDPad::dPadWithFrame(CCSpriteFrame *frame, float radius)
{
	SimpleDPad *pRet = new SimpleDPad();
	if (pRet && pRet->initWithFrame(frame, radius))
	{
		return pRet;
	}
	else
...
bool SimpleDPad::initWithFrame(CCSpriteFrame *frame, float radius)
{
	bool bRet = false;
	do
	{
		CC_BREAK_IF(!CCSprite::initWithSpriteFrame(frame));
